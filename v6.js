...
function v6GetGuitarCollapse(){try{const v=JSON.parse(localStorage.getItem('musicandbeats:ui:controls')||'{}');return !!v.guitarControls}catch{return false}}
function v6SetGuitarCollapse(val){try{const v=JSON.parse(localStorage.getItem('musicandbeats:ui:controls')||'{}');v.guitarControls=!!val;localStorage.setItem('musicandbeats:ui:controls',JSON.stringify(v))}catch{}}

function v6GuitarMarkup(context='play'){
  const collapsed=v6GetGuitarCollapse();
  return`<div class="v6-guitar-rig ${context==='record'?'v6-record-guitar':''}" data-guitar-rig="${context}">
    <div class="v6-guitar-hero">
      <div><small>CONNECTED GUITAR RIG</small><strong>${v6GuitarState.patch}</strong></div>
      <div style="display:flex;align-items:center;gap:10px">
        <span class="v6-signal-state ${v6GuitarState.connected?'live':''}">${v6GuitarState.connected?'Waiting for signal':'Input not connected'}</span>
        <button class="v39-focus-toggle v6-guitar-collapse-btn" type="button" aria-expanded="${!collapsed}">${collapsed?'<span>Show controls</span> <i>⌄</i>':'<span>Hide controls</span> <i>⌃</i>'}</button>
      </div>
    </div>
    <div class="v6-guitar-connect"><label>Audio input<select data-guitar-device><option value="">Default audio input</option></select></label><button data-guitar-connect type="button">${v6GuitarState.connected?'Reconnect':'Connect guitar'}</button><button data-guitar-monitor class="${v6GuitarState.monitor?'active':''}" type="button">Monitor ${v6GuitarState.monitor?'ON':'OFF'}</button></div>
    <div class="v6-guitar-meter"><span>INPUT</span><div class="v6-meter-track"><div class="v6-meter-fill"></div></div><span class="v6-meter-db">−60 dB</span></div>
    <div class="v6-guitar-collapsible ${collapsed?'v39-hidden':''}">
      <div class="v6-patch-row"><label>Amp patch<select data-guitar-patch>${Object.keys(V6_GUITAR_PATCHES).map(n=>`<option ${n===v6GuitarState.patch?'selected':''}>${n}</option>`).join('')}</select></label><div class="v6-knob"><span>Input <b>${Math.round(v6GuitarState.trim*100)}%</b></span><input data-guitar-param="trim" type="range" min="0.35" max="1.4" step=".01" value="${v6GuitarState.trim}"></div><div class="v6-knob"><span>Tone <b>${(v6GuitarState.tone/1000).toFixed(1)}k</b></span><input data-guitar-param="tone" type="range" min="1800" max="12000" step="100" value="${v6GuitarState.tone}"></div><div class="v6-knob"><span>Output <b>${Math.round(v6GuitarState.output*100)}%</b></span><input data-guitar-param="output" type="range" min="0" max="1.2" step=".01" value="${v6GuitarState.output}"></div></div>
      <div class="v6-pedalboard">${['drive','chorus','delay','reverb'].map(v6PedalMarkup).join('')}</div>
    </div>
  </div>`;
}
function v6BindGuitarRig(root){
  if(!root||root.dataset.v6bound==='1')return;
  root.dataset.v6bound='1';
  const dev=root.querySelector('[data-guitar-device]');
  root.querySelector('.v6-guitar-collapse-btn')?.addEventListener('click',()=>{
    const next=!v6GetGuitarCollapse();
    v6SetGuitarCollapse(next);
    const collapsible=root.querySelector('.v6-guitar-collapsible');
    const btn=root.querySelector('.v6-guitar-collapse-btn');
    if(collapsible)collapsible.classList.toggle('v39-hidden',next);
    if(btn){
      btn.setAttribute('aria-expanded',String(!next));
      btn.innerHTML=next?'<span>Show controls</span> <i>⌄</i>':'<span>Hide controls</span> <i>⌃</i>';
    }
  });
  root.querySelector('[data-guitar-connect]')?.addEventListener('click',async()=>{const ok=await v6ConnectGuitar(dev?.value||'');if(!ok)alert('Could not access the guitar input. Check browser microphone/audio permissions and your interface connection.')});
  root.querySelector('[data-guitar-monitor]')?.addEventListener('click',()=>{v6GuitarState.monitor=!v6GuitarState.monitor;v6ApplyGuitarState();v6SyncGuitarUIs()});
  root.querySelector('[data-guitar-patch]')?.addEventListener('change',e=>v6ApplyGuitarPatch(e.target.value));
  root.querySelectorAll('[data-guitar-param]').forEach(inp=>inp.addEventListener('input',()=>{const k=inp.dataset.guitarParam;v6GuitarState[k]=+inp.value;v6ApplyGuitarState();v6SyncGuitarUIs(root)}));
  root.querySelectorAll('.v6-pedal').forEach(p=>{const name=p.dataset.pedal;p.querySelector('[data-pedal-toggle]').addEventListener('click',()=>{v6GuitarState.pedals[name].on=!v6GuitarState.pedals[name].on;v6ApplyGuitarState();v6SyncGuitarUIs()});p.querySelector('[data-pedal-amount]').addEventListener('input',e=>{v6GuitarState.pedals[name].amount=+e.target.value;v6ApplyGuitarState();v6SyncGuitarUIs(p)})});
  v6PopulateGuitarDeviceMenus();
}
