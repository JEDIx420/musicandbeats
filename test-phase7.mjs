/**
 * Music & Beats — Non-invasive Phase 7 Verification Suite (Play-First)
 * Tests AppCore navigation & lifecycle, PlayUI view-models, Groove Box sequencer models,
...
import { grooveBox } from './src/groove-box.js';
...
console.log('--- 1. Testing AppCore Play-First Lifecycle & Navigation ---');
const app = new AppCore();
assert(app.currentScreen === 'play', 'AppCore initial screen is play (Play-First)');
...
console.log('\n--- 2. Testing PlayUI View Models & Performance Surfaces ---');
...
assert(leadVM.keyboardModel.blackKeys.length === 10, 'Lead VM keyboard has 10 black keys');
...
// Groove Box & Arp View Model
grooveBox.loadStyle('Trap', 4);
const grooveVM = play.getGrooveBoxViewModel();
assert(grooveVM.style === 'Trap', 'Groove Box style is Trap');
assert(grooveVM.energy === 4, 'Groove Box energy is 4');
assert(grooveVM.pattern.kick.length === 16, 'Groove Box 16-step kick present');

const arpVM = play.getArpViewModel();
assert(arpVM.rates.includes('1/64'), 'Arp VM rates includes 1/64');


console.log('\n--- 3. Testing Help Subsystem & Explain Controls ---');
...
console.log('\n--- 4. Testing Listener Cleanup & Zero Leakage ---');
