...
function v14LatchPad(pad){
  ...
  if(c.id==='#playChords'&&v6Arp?.enabled){v6StartArp({chord:c.chord,pad:c.pad,preset:c.preset,octave:c.octave,voicing:c.voicing});v14Latch={...c,arp:true,voices:[]}}
  else{const voices=v6StartSmartChord(c.chord,{voicing:c.voicing,octave:c.octave,preset:c.preset,velocity:.78});v14Latch={...c,arp:false,voices}}
...
function v14PaintHints(){
  ...
  document.querySelectorAll('.v6-smart-toolbar small').forEach(el=>{
    if(el.dataset.v14Hint===state)return;el.dataset.v14Hint=state;
...
document.addEventListener('pointerdown',e=>{
  ...
  /* A single tap on the already-held pad should not stack another copy of the same chord. */
  if(v14Latch&&v14Latch.id==='#'+v14SmartHostForPad(pad).id&&v14Latch.index===+pad.dataset.index){v14SuppressedPointers.add(e.pointerId);e.preventDefault();e.stopImmediatePropagation()}
},true);
...
v5TriggerNumberChord=function(index){
  ...
  if(c&&pad){
    ...
    if(v14Latch&&v14Latch.id==='#'+c.host.id&&v14Latch.index===index)return true;
  }
