...
async function sampleVoice(midi,name,outNode=null,vel=0.8){
  ...
  const v={midi,currentMidi:midi,velocity:vel,stopped:false,apply(gl=.02,vib=0){
    ...
    setTimeout(()=>{try{src.disconnect();g.disconnect()}catch{}},250);
  }};
...
async function startMidiLead(midi,vel=0.8,ch=1){try{await ensureAudio()}catch{}primeAudio();duck();const kId=midiKeyId(midi,ch);const existing=S.leadMidiVoices.get(kId);if(existing){existing.voice?.stop?.();S.leadMidiVoices.delete(kId)}const key=document.querySelector(`#v38Keyboard .v38-key[data-midi="${midi}"]`);if(key)key.classList.add('active');const pending={cancelled:false,key,midi,vel,ch};S.leadMidiPending.set(kId,pending);let voice=null;try{voice=await makeVoice(midi,null,vel)}catch(err){console.warn('startMidiLead error',err)}S.leadMidiPending.delete(kId);if(pending.cancelled||!voice){voice?.stop?.();if(key&&!isLeadMidiActive(midi))key.classList.remove('active');duck();return null}const entry={voice,key,midi,vel,ch};S.leadMidiVoices.set(kId,entry);duck();return entry}
...
window.addEventListener('blur',()=>{stopLead()},true);document.addEventListener('visibilitychange',()=>{if(document.hidden)stopLead()},true);
